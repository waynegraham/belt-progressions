    ...
    <main className={`min-h-screen bg-gradient-to-br ${track.theme.pageBg} p-4 md:p-8`}>
      <div className="mx-auto max-w-4xl space-y-6">
        <header className={`rounded-2xl border p-6 ${track.theme.cardBg} ${track.theme.border}`}>
          <p className="text-sm font-semibold uppercase tracking-wider text-zinc-600">Preparation</p>
          ...
          <Link
            href={`/${track.slug}`}
            className="mt-4 inline-flex h-10 items-center justify-center rounded-md border border-zinc-300 bg-white px-4 text-sm font-semibold text-zinc-800 hover:bg-zinc-50"
          >
            Back to Move List
          </Link>
        </header>

        <section className={`rounded-2xl border p-4 md:p-6 ${track.theme.cardBg} ${track.theme.border}`}>
          <ul className="space-y-4">
            {track.trainingRecommendations.map((recommendation) => (
              <li key={recommendation.title} className="rounded-xl border border-zinc-200 bg-white p-5">
